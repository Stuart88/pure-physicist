        ...
        public static readonly List<EquationItem> EquationsList = new List<EquationItem>
        {
            ...
            new EquationItem("Gravitational Potential", 80)
            {
                ...
                DerivationStepsLatex = new List<DerivationStep>
                {
                    ...
                    new DerivationStep(@"\text{Work done by gravity:}", 30, 25),
                    new DerivationStep(@"W = \int_{r_1}^{r_2} F_r \ dr", 80),
                    new DerivationStep(@"W = - \int_{r_1}^{r_2} \frac{G m_E m}{r^2} \ dr", 80),
                    new DerivationStep(@"\text{(negative as gravity acts toward earth)}", 50, 25),
            ...
            new EquationItem("Escape Velocity", 80)
            {
                ...
                {
                    new DerivationStep(@"\text{\underline{Derivation}}", 50),
                    new DerivationStep(@"\text{Kinetic energy must equal work done to}", 30, 25),
                    new DerivationStep(@"\text{escape to infinite distance}", 30, 25),
            ...
            new EquationItem("Kepler's 1st Law")
            {
                EquationLatex = @"\text{All planets move in an elliptical orbit \\ with one focus about a star}",
                FontSize = 25
            },
            new EquationItem("Kepler's 2nd Law")
            {
                ...
                DerivationStepsLatex = new List<DerivationStep>
                {
                    new DerivationStep(@"\text{A line connecting a planet to its}", 30, 30),
                    new DerivationStep(@"\text{central star sweeps an equal }", 30, 30),
                    new DerivationStep(@"\text{area for all changes in time.}", 30, 30),
